...
block UpEnd
  ...
  parameter Real proOnTim(
    ...
    "Threshold time to check if newly enabled boiler being operated by more than 5 minutes"
    annotation (Dialog(group="Enable next boiler"));
  ...
  parameter Real chaChiWatIsoTim(
    ...
    annotation (Dialog(group="Hot water isolation valve"));
  ...
  Buildings.Controls.OBC.CDL.Interfaces.BooleanInput uEnaChiWatIsoVal
    "Status of boiler hot water isolation valve control: true=enabled valve is fully open"
  ...
  Buildings.Controls.OBC.CDL.Interfaces.BooleanInput uChiWatReq[nBoi]
    "Hot water request status for each boiler"
  ...
  Buildings.Controls.OBC.CDL.Interfaces.BooleanInput uConWatReq[nBoi]
    "Condenser water request status for each boiler"
  ...
  Buildings.Controls.OBC.CDL.Interfaces.RealInput VMinBoiWat_setpoint(
    final unit="m3/s")
    "Minimum boiler water flow setpoint calculated from upstream process"
...
protected
  ...
  Buildings.Controls.OBC.ASHRAE.PrimarySystem.BoilerPlant.Staging.Processes.Subsequences.EnableBoiler
    enaChi(
    final nBoi=nBoi, final proOnTim=proOnTim)
    "Enable next boiler"
  ...
  Buildings.Controls.OBC.ASHRAE.PrimarySystem.BoilerPlant.MinimumFlowBypass.Subsequences.FlowSetpoint
    ...
    "Reset minimum hot water flow setpoint"
  ...
  Buildings.Controls.OBC.CDL.Continuous.LessEqualThreshold lesEquThr(
    final threshold=0.5)
    "Check if the disabled boiler is not requiring hot water"
  ...
  Buildings.Controls.OBC.CDL.Logical.Switch chiWatIso[nBoi]
  ...
  Buildings.Controls.OBC.CDL.Logical.Switch chiWatByp
    "Hot water bypass flow setpoint"
  ...
  Buildings.Controls.OBC.CDL.Logical.Latch lat1
    "Maintain ON signal when the boiler has been proven on"
  ...
  Buildings.Controls.OBC.CDL.Logical.Latch lat2
    "Maintain ON signal when the hot water isolation valve has been closed"
    annotation (Placement(transformation(extent={{120,-30},{140,-10}})));

  Buildings.Controls.OBC.CDL.Logical.Latch lat3
    "Maintain ON signal when the boiler head pressure control has been disabled"
...
equation
  ...
  connect(con3.y, minBoiWatSet.uStaDow)
  ...
  connect(uChi, minBoiWatSet.uChi)
    annotation (Line(points={{-220,150},{-170,150},{-170,-126},{38,-126}},
      color={255,0,255}));

  connect(nexDisChi, minBoiWatSet.nexDisChi)
    annotation (Line(points={{-220,70},{-180,70},{-180,-131},{38,-131}},
      color={255,127,0}));

  connect(uOnOff, minBoiWatSet.uOnOff)
  ...
  connect(minBoiWatSet.yChiWatMinFloSet, chiWatByp1.u1)
    annotation (Line(points={{62,-130},{70,-130},{70,-122},{98,-122}},
      color={0,0,127}));

  connect(VMinBoiWat_setpoint, chiWatByp1.u3)
    annotation (Line(points={{-220,-200},{80,-200},{80,-138},{98,-138}},
      color={0,0,127}));

  connect(VMinBoiWat_setpoint, chiWatByp.u1)
  ...
  connect(nexEnaChi, minBoiWatSet.nexEnaChi)
  ...
  connect(lat1.y, minBoiWatSet.uStaUp)
  ...
  connect(lat3.y, minBoiWatSet.uSubCha)
...
<p>
Block that controls devices at the ending step of boiler staging up process.
...
<p>
For the stage-up process that does not require a smaller boiler being disabled
and a larger boiler being enabled (<code>uOnOff=false</code>),
</p>
<ul>
<li>
Start the next stage boiler (<code>nexEnaChi</code>) after the hot water 
...
is enabled (<code>uOnOff=true</code>), after starting the next stage boiler 
specified above, do following:
</p>
<ol>
<li>
Wait 5 minutes (<code>proOnTim</code>) for the newly enabled boiler to prove that 
is operating correctly, then shut off the small boiler (<code>nexDisChi</code>). 
This is implemented in block <code>enaChi</code>. See
<a href=\"modelica://Buildings.Controls.OBC.ASHRAE.PrimarySystem.BoilerPlant.Staging.Processes.Subsequences.EnableBoiler\">
Buildings.Controls.OBC.ASHRAE.PrimarySystem.BoilerPlant.Staging.Processes.Subsequences.EnableBoiler</a>
for more decriptions.
...
<li>
When the controller of the smaller boiler being shut off indicates no request
for hot water flow (<code>uChiWatReq=false</code>), slowly close the boiler's 
hot water isolation valve to avoid a sudden change in flow through other 
operating boilers.
...
<li>
When the controller of the smaller boiler being shut off indicates no request for 
condenser water flow (<code>uConWatReq=false</code>), disable the boiler's head 
...
This is implemented in block <code>minBoiWatSet</code>. See
...
<li>
Block <code>minBypSet</code> will then check if the new hot water flow setpoint
