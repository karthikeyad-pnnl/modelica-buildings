...
model UpEndWithOff
  ...
  Buildings.Controls.OBC.ASHRAE.PrimarySystem.BoilerPlant.Staging.Processes.Subsequences.UpEnd
    ...
    "End staging up process which does require one boiler on and another boiler off"
...
protected
  ...
  Buildings.Controls.OBC.CDL.Continuous.Sources.Constant nexEnaChi1(
    final k=2)
    "Next enable boiler"
  ...
  Buildings.Controls.OBC.CDL.Logical.Pre chiOneSta1(
    final pre_u_start=true)
    "Boiler one status"
    annotation (Placement(transformation(extent={{140,120},{160,140}})));

  Buildings.Controls.OBC.CDL.Logical.Pre chiTwoSta1(
  ...
  Buildings.Controls.OBC.CDL.Continuous.Sources.Constant zer4(
    final k=0)
    "Constant zero"
    annotation (Placement(transformation(extent={{-180,-40},{-160,-20}})));

  Buildings.Controls.OBC.CDL.Continuous.Sources.Constant nexDisChi2(
    final k=1)
    "Next disable boiler"
  ...
  Buildings.Controls.OBC.CDL.Logical.Sources.Constant enaHeaCon(
    final k=true)
  ...
  Buildings.Controls.OBC.CDL.Continuous.Sources.Constant chiWatFlo1(
    final k=1)
    "Hot water flow rate"
    annotation (Placement(transformation(extent={{-180,-210},{-160,-190}})));

  Buildings.Controls.OBC.CDL.Continuous.Sources.Constant chiWatFlo2(
    final k=1.667)
    "Minimum hot water flow setpoint calculated from upstream process"
...
equation
  ...
  connect(chiWatFlo2.y, endUp1.VMinBoiWat_setpoint)
...
annotation (
 experiment(StopTime=1200, Tolerance=1e-06),
  __Dymola_Commands(file="modelica://Buildings/Resources/Scripts/Dymola/Controls/OBC/ASHRAE/PrimarySystem/BoilerPlant/Staging/Processes/Subsequences/Validation/UpEndWithOff.mos"
    "Simulate and plot"),
