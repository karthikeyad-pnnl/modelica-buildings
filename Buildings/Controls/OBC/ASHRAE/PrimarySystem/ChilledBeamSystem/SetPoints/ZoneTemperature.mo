...
block ZoneTemperature
  ...
  CDL.Interfaces.IntegerInput uOpeMod "Operating mode signal"
    annotation (Placement(transformation(extent={{-140,-20},{-100,20}})));

  CDL.Interfaces.RealOutput TZonHeaSet "Zone heating setpoint temperature"
    annotation (Placement(transformation(extent={{100,40},{140,80}})));
  CDL.Interfaces.RealOutput TZonCooSet "Zone cooling setpoint temperature"
    annotation (Placement(transformation(extent={{100,-80},{140,-40}})));

  CDL.Continuous.Sources.Constant con(final k=zonOccHeaSet)
    "Zone occupied heating setpoint"
    annotation (Placement(transformation(extent={{-80,70},{-60,90}})));
  CDL.Continuous.Sources.Constant con1(final k=zonUnoccHeaSet)
    "Zone unoccupied heating setpoint"
    annotation (Placement(transformation(extent={{-80,30},{-60,50}})));
  CDL.Continuous.Sources.Constant con2(final k=zonOccCooSet)
    "Zone occupied cooling setpoint"
    annotation (Placement(transformation(extent={{-80,-50},{-60,-30}})));
  CDL.Continuous.Sources.Constant con3(final k=zonUnoccCooSet)
    "Zone unoccupied cooling setpoint"
    annotation (Placement(transformation(extent={{-80,-90},{-60,-70}})));
  CDL.Integers.Equal intEqu "Check if current mode is occupied"
    annotation (Placement(transformation(extent={{0,-10},{20,10}})));
  CDL.Integers.Sources.Constant conInt(final k=Buildings.Controls.OBC.ASHRAE.PrimarySystem.ChilledBeamSystem.Types.OperationModeTypes.occupied)
    "Occupied mode signal"
    annotation (Placement(transformation(extent={{-40,-30},{-20,-10}})));
  CDL.Logical.Switch swi "Switch"
    annotation (Placement(transformation(extent={{40,50},{60,70}})));
  CDL.Logical.Switch swi1 "Switch"
