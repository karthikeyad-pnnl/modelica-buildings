...
block EnergyWheel "This block commands the energy recovery wheel and associated bypass dampers."
  ...
  Buildings.Controls.OBC.CDL.Interfaces.RealInput oaT "Outside air temperature sensor." annotation(
    Placement(transformation(extent = {{-142, -38}, {-102, 2}}), iconTransformation(extent = {{-142, -38}, {-102, 2}})));
  Buildings.Controls.OBC.CDL.Interfaces.RealInput raT "Return air temperature sensor." annotation(
    Placement(transformation(extent = {{-142, 0}, {-102, 40}}), iconTransformation(extent = {{-142, -2}, {-102, 38}})));
  Buildings.Controls.OBC.CDL.Interfaces.BooleanInput ecoMode "True when economizer mode is active." annotation(
    Placement(transformation(extent = {{-142, 30}, {-102, 70}}), iconTransformation(extent = {{-142, 28}, {-102, 68}})));
  Buildings.Controls.OBC.CDL.Interfaces.RealInput erwT "Energy recovery wheel supply air temperature." annotation(
    Placement(transformation(extent = {{-142, -74}, {-102, -34}}), iconTransformation(extent = {{-142, -68}, {-102, -28}})));
  Buildings.Controls.OBC.CDL.Interfaces.RealInput supPrimSP "Primary supply air temperature set point." annotation(
    Placement(transformation(extent = {{-142, -106}, {-102, -66}}), iconTransformation(extent = {{-142, -98}, {-102, -58}})));
  // ---outputs---
  Buildings.Controls.OBC.CDL.Interfaces.BooleanOutput erwStart "Command to start the energy recovery wheel." annotation(
    Placement(transformation(extent = {{102, 0}, {142, 40}}), iconTransformation(extent = {{102, -20}, {142, 20}})));
  Buildings.Controls.OBC.CDL.Interfaces.RealOutput erwSpeed(final unit = "1", final min = 0, final max = 1) "Energy recovery wheel speed command." annotation(
    Placement(transformation(extent = {{102, -96}, {142, -56}}), iconTransformation(extent = {{102, -80}, {142, -40}})));
  Buildings.Controls.OBC.CDL.Interfaces.BooleanOutput bypDam "Bypass damper command; true when commanded full open." annotation(
    Placement(transformation(extent = {{102, 46}, {142, 86}}), iconTransformation(extent = {{102, 40}, {142, 80}})));
  Buildings.Controls.OBC.CDL.Continuous.Add difference(final k2 = -1) "Subtract outside air temperature from return air temperature." annotation(
    Placement(transformation(extent = {{-90, -10}, {-70, 10}})));
  Buildings.Controls.OBC.CDL.Continuous.Abs abs "Absolute value of OAT-RAT difference." annotation(
    Placement(transformation(extent = {{-62, -10}, {-42, 10}})));
  Buildings.Controls.OBC.CDL.Continuous.Greater gre "True if |OAT-RAT| > the recovery set point." annotation(
    Placement(transformation(extent = {{-26, -10}, {-6, 10}})));
  Buildings.Controls.OBC.CDL.Continuous.Sources.Constant conRecovery(final k = recSet) "Recovery set point." annotation(
    Placement(transformation(extent = {{-62, -46}, {-42, -26}})));
  Buildings.Controls.OBC.CDL.Logical.Latch lat(pre_y_start = true) "Latches true when |RAT-OAT| > set point; resets when 
        |RAT-OAT| < set point for delay period." annotation(
    Placement(transformation(extent = {{30, -10}, {50, 10}})));
  Buildings.Controls.OBC.CDL.Continuous.Less les "True if |OAT-RAT| < recovery set point." annotation(
    Placement(transformation(extent = {{-26, -38}, {-6, -18}})));
  Buildings.Controls.OBC.CDL.Logical.TrueDelay truDel(delayTime = recSetDelay, delayOnInit = true) "Recovery set point delay before disabling energy wheel." annotation(
    Placement(transformation(extent = {{2, -38}, {22, -18}})));
  Buildings.Controls.OBC.CDL.Logical.And3 and3 "Logical AND; true when fan is proven, economizer mode is off, 
        and ERW temperature start conditions are met." annotation(
    Placement(transformation(extent = {{62, 10}, {82, 30}})));
  Buildings.Controls.OBC.CDL.Logical.Not not1 "Logical NOT; true when economizer mode is off." annotation(
    Placement(transformation(extent = {{-26, 40}, {-6, 60}})));
  Buildings.Controls.OBC.CDL.Continuous.Max max "Outputs maximum value of two ERW temperature PI loops." annotation(
    Placement(transformation(extent = {{-62, -78}, {-42, -58}})));
  Buildings.Controls.OBC.CDL.Logical.Switch swi "Logical switch outputs ERW temperature PI maximum output 
        when erwStart command is true." annotation(
    Placement(transformation(extent = {{66, -86}, {86, -66}})));
  ...
  Buildings.Controls.OBC.CDL.Logical.And and1 "Logical AND; true when fan is proven and unit is in economizer mode." annotation(
    Placement(transformation(extent = {{-64, 56}, {-44, 76}})));
  Buildings.Controls.OBC.CDL.Logical.Or or2 "Logical OR." annotation(
...
equation
  ...
  connect(difference.u1, raT) annotation(
    Line(points = {{-92, 6}, {-96, 6}, {-96, 20}, {-122, 20}}, color = {0, 0, 127}));
  connect(difference.u2, oaT) annotation(
    Line(points = {{-92, -6}, {-96, -6}, {-96, -18}, {-122, -18}}, color = {0, 0, 127}));
  connect(abs.y, gre.u1) annotation(
    Line(points = {{-40, 0}, {-28, 0}}, color = {0, 0, 127}));
  connect(gre.y, lat.u) annotation(
    Line(points = {{-4, 0}, {28, 0}}, color = {255, 0, 255}));
  connect(conRecovery.y, gre.u2) annotation(
    Line(points = {{-40, -36}, {-32, -36}, {-32, -8}, {-28, -8}}, color = {0, 0, 127}));
  connect(conRecovery.y, les.u2) annotation(
    Line(points = {{-40, -36}, {-28, -36}}, color = {0, 0, 127}));
  connect(abs.y, les.u1) annotation(
    Line(points = {{-40, 0}, {-36, 0}, {-36, -28}, {-28, -28}}, color = {0, 0, 127}));
  connect(les.y, truDel.u) annotation(
    Line(points = {{-4, -28}, {0, -28}}, color = {255, 0, 255}));
  connect(truDel.y, lat.clr) annotation(
    Line(points = {{24, -28}, {26, -28}, {26, -6}, {28, -6}}, color = {255, 0, 255}));
  connect(lat.y, and3.u3) annotation(
    Line(points = {{52, 0}, {56, 0}, {56, 12}, {60, 12}}, color = {255, 0, 255}));
