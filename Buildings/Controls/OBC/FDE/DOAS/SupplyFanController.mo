...
block SupplyFanController "This block manages start, stop, status, and speed of the supply fan."
  ...
  Buildings.Controls.OBC.CDL.Interfaces.RealInput mostOpenDam if vvUnit "Most open damper position from all terminal units served." annotation(
    Placement(transformation(extent = {{-142, -24}, {-102, 16}}), iconTransformation(extent = {{-140, 14}, {-100, 54}})));
  Buildings.Controls.OBC.CDL.Interfaces.BooleanInput supFanStatus "True when supply fan is proven on." annotation(
    Placement(transformation(extent = {{-142, 22}, {-102, 62}}), iconTransformation(extent = {{-140, -56}, {-100, -16}})));
  Buildings.Controls.OBC.CDL.Interfaces.RealInput DDSP "Down duct static pressure measurement." annotation(
    Placement(transformation(extent = {{-142, -106}, {-102, -66}}), iconTransformation(extent = {{-140, -92}, {-100, -52}})));
  // ---outputs---
  Buildings.Controls.OBC.CDL.Interfaces.BooleanOutput supFanStart "Command supply fan to start when true." annotation(
    Placement(transformation(extent = {{102, 60}, {142, 100}}), iconTransformation(extent = {{100, 32}, {140, 72}})));
  Buildings.Controls.OBC.CDL.Interfaces.BooleanOutput supFanProof "True when supply fan is proven on" annotation(
    Placement(transformation(extent = {{102, 22}, {142, 62}}), iconTransformation(extent = {{100, -20}, {140, 20}})));
  Buildings.Controls.OBC.CDL.Interfaces.RealOutput supFanSpeed "Supply fan speed command" annotation(
    Placement(transformation(extent = {{102, -50}, {142, -10}}), iconTransformation(extent = {{100, -64}, {140, -24}})));
  Buildings.Controls.OBC.CDL.Continuous.Sources.Constant dampSet(final k = damSet) if vvUnit "Most open damper position set point." annotation(
    Placement(transformation(extent = {{-98, 4}, {-78, 24}})));
  Buildings.Controls.OBC.CDL.Logical.Sources.Constant conVVunit(final k = vvUnit) "True when unit serves variable volume system." annotation(
    Placement(transformation(extent = {{-72, -32}, {-52, -12}})));
  Buildings.Controls.OBC.CDL.Continuous.Sources.Constant cvDDSPsetpt(final k = cvDDSPset) if not vvUnit "DDSP set point for constant volume systems." annotation(
    Placement(transformation(extent = {{-72, -64}, {-52, -44}})));
  CDL.Logical.Switch swi "Swtich passes fan speed set point when true; 0 when false." annotation(
...
equation
  connect(supFanStart, occ) annotation(
    Line(points = {{122, 80}, {-122, 80}}, color = {255, 0, 255}));
  connect(supFanStatus, supFanProof) annotation(
    Line(points = {{-122, 42}, {122, 42}}, color = {255, 0, 255}));
