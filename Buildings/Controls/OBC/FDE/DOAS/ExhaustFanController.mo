...
block ExhaustFanController "This block manages start, stop, and speed of the exhaust fan."
  ...
  Buildings.Controls.OBC.CDL.Interfaces.RealInput bldgSP "Building static pressure" annotation(
    Placement(transformation(extent = {{-142, -28}, {-102, 12}}), iconTransformation(extent = {{-140, -84}, {-100, -44}})));
  Buildings.Controls.OBC.CDL.Interfaces.BooleanInput exhFanProof "True when exhaust fan is proven on." annotation(
    Placement(transformation(extent = {{-142, -62}, {-102, -22}}), iconTransformation(extent = {{-140, -20}, {-100, 20}})));
  // ---outputs---
  Buildings.Controls.OBC.CDL.Interfaces.BooleanOutput exhFanStart "Command exhaust fan to start when true." annotation(
    Placement(transformation(extent = {{102, 34}, {142, 74}}), iconTransformation(extent = {{100, 40}, {140, 80}})));
  Buildings.Controls.OBC.CDL.Interfaces.RealOutput exhFanSpeed "Exhaust fan speed command" annotation(
    Placement(transformation(extent = {{102, -16}, {142, 24}}), iconTransformation(extent = {{100, -80}, {140, -40}})));
  Buildings.Controls.OBC.CDL.Continuous.Sources.Constant bldgStaticSP(final k = bldgSPset) "Building static pressure set point." annotation(
    Placement(transformation(extent = {{-40, 2}, {-20, 22}})));
  Buildings.Controls.OBC.CDL.Logical.Switch swi "Logical switch is true when fan status is proven." annotation(
...
equation
  connect(exhFanStart, supFanProof) annotation(
    Line(points = {{122, 54}, {-122, 54}}, color = {255, 0, 255}));
