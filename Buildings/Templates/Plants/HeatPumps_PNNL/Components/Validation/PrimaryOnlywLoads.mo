...
model PrimaryOnlywLoads "Control Box Test"
  ...
  Fluid.MixingVolumes.MixingVolume vol1(
    redeclare package Medium = Buildings.Media.Water,
    m_flow_nominal=datHpAwNrv.mChiWatHp_flow_nominal,
    V=datHpAwNrv.mChiWatHp_flow_nominal*600/1000,
    nPorts=3) annotation (Placement(transformation(extent={{334,28},{354,48}})));
  ...
  HeatTransfer.Sources.PrescribedHeatFlow preHeaFlo1
    annotation (Placement(transformation(extent={{266,126},{286,146}})));
  ...
  Buildings.Controls.OBC.CDL.Reals.MultiplyByParameter gai1(k=10)
    annotation (Placement(transformation(extent={{430,40},{450,60}})));
  Buildings.Controls.OBC.CDL.Reals.Sources.Constant con1(k=15.55)
    annotation (Placement(transformation(extent={{360,0},{380,20}})));
  Controls.ExternalEnergyOpenLoop
                          externalEnergyOpenLoop
  ...
  Buildings.Controls.OBC.CDL.Logical.Sources.Constant con6(k=false)
    annotation (Placement(transformation(extent={{398,-40},{418,-20}})));
  Buildings.Controls.OBC.CDL.Reals.Sources.Constant y1(k=-41000)
    "Pump speed command"
    annotation (Placement(transformation(extent={{-500,100},{-480,120}})));
  Buildings.Controls.OBC.CDL.Reals.Sources.Constant y4(k=0)
    "Pump speed command"
  ...
  Buildings.Controls.OBC.CDL.Reals.MultiplyByParameter gai3(k=-1)
    annotation (Placement(transformation(extent={{426,88},{446,108}})));
  Buildings.Controls.OBC.CDL.Reals.AddParameter addPar1(p=1)
    annotation (Placement(transformation(extent={{466,88},{486,108}})));
...
connect(resCoo.dpSet[1], bus_sensor.uDpCooSet);
  ...
  connect(TRetCoo.port_a, vol1.ports[1]) annotation (Line(points={{248,22},{
          342.667,22},{342.667,28}},
                             color={0,127,255}));
  connect(TSupCoo.port_b, vol1.ports[2]) annotation (Line(points={{280,90},{328,
          90},{328,24},{344,24},{344,28}}, color={0,127,255}));
  connect(preHeaFlo.port, vol.heatPort) annotation (Line(points={{-420,110},{-360,
          110},{-360,86},{-412,86}}, color={191,0,0}));
  connect(preHeaFlo1.port, vol1.heatPort) annotation (Line(points={{286,136},{292,
          136},{292,38},{334,38}}, color={191,0,0}));
  connect(senTem.port, vol.ports[1]) annotation (Line(points={{-450,80},{-450,
          72},{-426,72},{-426,76},{-403.333,76}},
                                              color={0,127,255}));
  connect(vol1.ports[3], senTem1.port) annotation (Line(points={{345.333,28},{
          342,28},{342,22},{358,22},{358,40},{370,40}},
                                                    color={0,127,255}));
  connect(senTem.T, conPID.u_m) annotation (Line(points={{-443,90},{-440,90},{-440,
          10},{-420,10},{-420,18}}, color={0,0,127}));
  connect(conPID.y, gai.u) annotation (Line(points={{-408,30},{-394,30},{-394,0},
          {-382,0}}, color={0,0,127}));
  connect(con.y, conPID.u_s)
    annotation (Line(points={{-478,30},{-432,30}}, color={0,0,127}));
  connect(conPID1.y, gai1.u)
    annotation (Line(points={{422,50},{428,50}}, color={0,0,127}));
  connect(con1.y, conPID1.u_s) annotation (Line(points={{382,10},{392,10},{392,50},
          {398,50}}, color={0,0,127}));
  connect(senTem1.T, conPID1.u_m) annotation (Line(points={{377,50},{386,50},{386,
          38},{410,38}}, color={0,0,127}));
  connect(externalEnergyOpenLoop.bus, bus) annotation (Line(
      ...
      extent={{-6,3},{-6,3}},
      horizontalAlignment=TextAlignment.Right));
  connect(opeModCon.yOpeMod, bus.uOpeMod) annotation (Line(points={{-78,178},{38,
          178},{38,290}}, color={255,127,0}), Text(
      string="%second",
      index=1,
      extent={{6,3},{6,3}},
      horizontalAlignment=TextAlignment.Left));
  ...
  connect(gai1.y, reaToInt1.u)
    annotation (Line(points={{452,50},{458,50}}, color={0,0,127}));
  connect(reaToInt.y, addInt.u1) annotation (Line(points={{-318,-32},{-290,-32},
          {-290,-24},{-282,-24}}, color={255,127,0}));
  connect(reaToInt1.y, addInt.u2) annotation (Line(points={{482,50},{498,50},{498,
          -138},{-290,-138},{-290,-36},{-282,-36}}, color={255,127,0}));
  ...
  connect(con6.y, resCoo.u1StaPro) annotation (Line(points={{420,-30},{440,-30},
          {440,-16},{458,-16}}, color={255,0,255}));
  connect(reaToInt1.y, resCoo.nReqRes) annotation (Line(points={{482,50},{488,50},
          {488,6},{450,6},{450,-4},{458,-4}}, color={255,127,0}));
  connect(ena.y1, resCoo.u1Ena) annotation (Line(points={{-158,190},{-156,190},{
          -156,106},{-248,106},{-248,-10},{458,-10}}, color={255,0,255}));
  ...
  connect(y4.y, preHeaFlo1.Q_flow) annotation (Line(points={{242,140},{258,140},
          {258,136},{266,136}}, color={0,0,127}));
  connect(conPID.y, gai2.u) annotation (Line(points={{-408,30},{-394,30},{-394,50},
          {-382,50}}, color={0,0,127}));
  connect(gai2.y, addPar.u)
    annotation (Line(points={{-358,50},{-342,50}}, color={0,0,127}));
  ...
  connect(resHea.TSupSet, opeModCon.THeaSupSet) annotation (Line(points={{-258,-76},
          ...
          182}}, color={0,0,127}));
  connect(resCoo.TSupSet, opeModCon.TChiSupSet) annotation (Line(points={{482,-16},
          {192,-16},{192,166},{-102,166}}, color={0,0,127}));
  ...
  connect(TSupHea.port_b, vol.ports[2]) annotation (Line(points={{-328,24},{-348,
          24},{-348,76},{-402,76}}, color={0,127,255}));
  connect(TRetHea.port_b, jun6.port_1) annotation (Line(points={{-320,98},{-254,
          98},{-254,94},{-222,94}}, color={0,127,255}));
  connect(vol.ports[3], TRetHea.port_a) annotation (Line(points={{-400.667,76},
  ...
  connect(gai3.y, addPar1.u)
    annotation (Line(points={{448,98},{464,98}}, color={0,0,127}));
  connect(conPID1.y, gai3.u) annotation (Line(points={{422,50},{358,50},{358,98},
          {424,98}}, color={0,0,127}));
  connect(addPar1.y, bus_CooVal.y) annotation (Line(points={{488,98},{496,98},{
          496,182},{170,182},{170,142}}, color={0,0,127}), Text(
      string="%second",
      index=1,
      extent={{-6,3},{-6,3}},
      horizontalAlignment=TextAlignment.Right));
