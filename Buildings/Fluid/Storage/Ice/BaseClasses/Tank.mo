...
model Tank "Block to compute the tank heat transfer"
  ...
  Modelica.Blocks.Interfaces.RealInput TInCha(final unit="K", displayUnit="degC")
    "Inlet temperature"
    annotation (Placement(transformation(extent={{-140,40},{-100,80}})));
  Modelica.Blocks.Interfaces.RealInput TOutCha(final unit="K", displayUnit="degC")
    "Outlet temperature"
  ...
  Buildings.Fluid.Storage.Ice.BaseClasses.StateOfCharge SOCCalc(
...
// protected
  ...
  Buildings.Fluid.Storage.Ice.BaseClasses.LMTDStar lmtdStaCha(final TFre=per.TFre,
      final dT_nominal=per.dT_nominal)
    annotation (Placement(transformation(extent={{-48,60},{-28,80}})));

  LMTDStar lmtdStaDis(final TFre=per.TFre, final dT_nominal=per.dT_nominal)
    annotation (Placement(transformation(extent={{-50,90},{-30,110}})));
  Modelica.Blocks.Interfaces.RealInput TInDis(final unit="K", displayUnit="degC")
    "Inlet temperature" annotation (Placement(transformation(extent={{-140,80},{
            -100,120}}), iconTransformation(extent={{-140,100},{-100,140}})));
  Modelica.Blocks.Interfaces.RealInput TOutDis(final unit="K", displayUnit="degC")
    "Outlet temperature" annotation (Placement(transformation(extent={{-140,-50},
            {-100,-10}}), iconTransformation(extent={{-140,-80},{-100,-40}})));
equation
  connect(norQSta.qNor, QCoe_flow.u)
    annotation (Line(points={{29,80},{36,80}}, color={0,0,127}));
  connect(QThe_flow.y, SOCCalc.Q_flow)
    annotation (Line(points={{21,-60},{28,-60}}, color={0,0,127}));
  connect(QMin_flow.y, QThe_flow.u1) annotation (Line(points={{-19,-40},{-8,-40},
          {-8,-52},{-2,-52}}, color={0,0,127}));
  connect(QCoe_flow.y, QMin_flow.u2) annotation (Line(points={{59,80},{60,80},{60,
          -20},{-56,-20},{-56,-46},{-42,-46}},                     color={0,0,127}));
  connect(QCoe_flow.y, QMax_flow.u1) annotation (Line(points={{59,80},{60,80},{60,
          -20},{-56,-20},{-56,-74},{-42,-74}},                     color={0,0,127}));
  connect(SOCCalc.SOC, SOC) annotation (Line(points={{51,-60},{64,-60},{64,0},{110,
          0}}, color={0,0,127}));
  connect(QMax_flow.y, QThe_flow.u3) annotation (Line(points={{-19,-80},{-8,-80},
          {-8,-68},{-2,-68}}, color={0,0,127}));
  connect(lmtdStaCha.lmtdSta, norQSta.lmtdStaCha) annotation (Line(points={{-27,
          70},{-10,70},{-10,74},{6,74}}, color={0,0,127}));
  connect(SOCCalc.SOC, norQSta.SOC) annotation (Line(points={{51,-60},{64,-60},{
          64,0},{0,0},{0,80},{6,80}}, color={0,0,127}));
  connect(norQSta.canMelt, canMelt.y) annotation (Line(points={{6,92},{-20,92},{
          -20,40},{-28,40}},
                           color={255,0,255}));
  connect(canFreeze.y, norQSta.canFreeze) annotation (Line(points={{-28,10},{-12,
          10},{-12,86},{6,86}},  color={255,0,255}));
  connect(canFreeze.y, QThe_flow.u2) annotation (Line(points={{-28,10},{-12,10},
          {-12,-60},{-2,-60}}, color={255,0,255}));
  connect(SOCCalc.QEff_flow, Q_flow) annotation (Line(points={{51,-54},{62,-54},
          {62,40},{110,40}}, color={0,0,127}));
  connect(lmtdStaCha.TIn, TInCha) annotation (Line(points={{-50,74},{-90,74},{-90,
          60},{-120,60}}, color={0,0,127}));
  connect(canFreeze.u, TInCha) annotation (Line(points={{-52,10},{-90,10},{-90,60},
          {-120,60}}, color={0,0,127}));
  connect(lmtdStaCha.TOut, TOutCha) annotation (Line(points={{-50,66},{-80,66},{
          -80,0},{-120,0}}, color={0,0,127}));
  connect(TInCha, canMelt.u) annotation (Line(points={{-120,60},{-90,60},{-90,40},
          {-52,40}}, color={0,0,127}));
  connect(QLim_flow, QMin_flow.u1) annotation (Line(points={{-120,-60},{-60,-60},
          {-60,-34},{-42,-34}}, color={0,0,127}));
  connect(QLim_flow, QMax_flow.u2) annotation (Line(points={{-120,-60},{-60,-60},
          {-60,-86},{-42,-86}}, color={0,0,127}));
  connect(SOCCalc.SOC, masIce.u)
    annotation (Line(points={{51,-60},{64,-60},{64,-70},{70,-70}},
                                                 color={0,0,127}));
  connect(masIce.y, mIce) annotation (Line(points={{93,-70},{96,-70},{96,-40},{110,
          -40}},     color={0,0,127}));
  connect(TInDis, lmtdStaDis.TIn) annotation (Line(points={{-120,100},{-80,100},
          {-80,104},{-52,104}}, color={0,0,127}));
  connect(TOutDis, lmtdStaDis.TOut) annotation (Line(points={{-120,-30},{-70,-30},
          {-70,96},{-52,96}}, color={0,0,127}));
  connect(lmtdStaDis.lmtdSta, norQSta.lmtdStaDis) annotation (Line(points={{-29,
          100},{-6,100},{-6,68},{6,68}}, color={0,0,127}));
