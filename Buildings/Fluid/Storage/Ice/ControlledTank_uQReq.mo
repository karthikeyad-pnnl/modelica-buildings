...
model ControlledTank_uQReq
  "Ice tank with performance based on performance curves and built-in control for outlet temperature"
  extends Buildings.Fluid.Storage.Ice.Tank(limQ_flow(y=if (tanHeaTra.canMelt.y and uQReq<0)
           then max(QMax, uQReq) elseif (tanHeaTra.canFreeze.y and uQReq>0)
           then min(QMax, uQReq) else
           0));
